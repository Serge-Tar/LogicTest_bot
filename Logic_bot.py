...
#----------
    ...
    #        # Выводим категорию, в которую попал пользователь
        ...
        total_score = 0  # Обнуляем счётчик
        question_number = 0 # Обнуляем номер вопроса
        # задаем кнопку клавиатуры "Викторина", для ее запуска
        murkup = ReplyKeyboardMarkup(one_time_keyboard=True, resize_keyboard=True)
        murkup.add("Викторина")
        bot.send_message(chat_id, "Извините произошла ошибка!( Пожалуйста, начните Тестирование снова  с помощью команды /start", reply_markup=murkup)
